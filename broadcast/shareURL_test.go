...
func TestChannel_ShareURL_PasswordRngError(t *testing.T) {
	c, _, err := NewChannel("ABC", "B", Secret, 512, csprng.NewSystemRNG())
	if err != nil {
		t.Fatalf("Failed to create new channel: %+v", err)
	}
...
// Tests that GetURLType returns the expected PrivacyLevel for every type
// of URL.
...
// Error path: Tests that GetURLType returns an error for an invalid host.
func TestGetShareUrlType_ParseError(t *testing.T) {
	host := "invalidHost\x7f"
	c, err := GetURLType(host)
	if err == nil {
		t.Errorf("Expected error for invalid host URL: %+v", c)
	}
}

// Error path: Tests that GetURLType returns errors for a list of invalid
// URLs.
...
// Channel.encodePublicURL and decoded to a new channel using
// Channel.decodePublicURL and that it matches the original.
func TestChannel_encodePublicShareURL_decodePublicShareURL(t *testing.T) {
	...
	urlValues = c.encodePublicURL(urlValues)

	var newChannel Channel
	err = newChannel.decodePublicURL(urlValues)
	if err != nil {
...
// Tests that a channel can be encoded to a URL using
// Channel.encodePrivateURL and decoded to a new channel using
// Channel.decodePrivateURL and that it matches the original.
func TestChannel_encodePrivateShareURL_decodePrivateShareURL(t *testing.T) {
	...
	maxUses := 12
	urlValues := make(goUrl.Values)
	urlValues = c.encodePrivateURL(urlValues, password, maxUses, rng, share)

	var newChannel Channel
	loadedMaxUses, err := newChannel.decodePrivateURL(urlValues, password, share)
	if err != nil {
		t.Errorf("Error decoding URL values: %+v", err)
	}
...
// Error path: Tests Channel.decodePrivateURL returns the expected error
...
func TestChannel_decodePrivateShareURL(t *testing.T) {
	...
	_, err := newChannel.decodePrivateURL(urlValues, "", share)
	if err == nil || !strings.Contains(err.Error(), expectedErr) {
...
// Tests that a channel can be encoded to a URL using
// Channel.encodeSecretURL and decoded to a new channel using
// Channel.decodeSecretURL and that it matches the original.
func TestChannel_encodeSecretShareURL_decodeSecretShareURL(t *testing.T) {
	...
	maxUses := 2
	urlValues := make(goUrl.Values)
	urlValues = c.encodeSecretURL(urlValues, password, maxUses, rng, share)

	var newChannel Channel
	loadedMaxUses, err := newChannel.decodeSecretURL(urlValues, password, share)
	if err != nil {
		t.Errorf("Error decoding URL values: %+v", err)
	}
...
// Tests that a channel marshalled with Channel.marshalPrivateURLSecrets
// and unmarshalled with Channel.unmarshalPrivateURLSecrets matches the
...
func TestChannel_marshalPrivateShareUrlSecrets_unmarshalPrivateShareUrlSecrets(
	...
	maxUses := 5
	data := c.marshalPrivateURLSecrets(maxUses)

	var newChannel Channel
	unmarshalledMaxUses, err := newChannel.unmarshalPrivateURLSecrets(data)
	if err != nil {
...
// Tests that a channel marshalled with Channel.marshalSecretURLSecrets and
// unmarshalled with Channel.unmarshalSecretURLSecrets matches the
...
func TestChannel_marshalSecretShareUrlSecrets_unmarshalSecretShareUrlSecrets(
	...
	maxUses := 5
	data := c.marshalSecretURLSecrets(maxUses)

	var newChannel Channel
	unmarshalledMaxUses, err := newChannel.unmarshalSecretURLSecrets(data)
	if err != nil {
...
func TestChannel_encodePrivateInviteURL_decodePrivateInviteURL(t *testing.T) {
	...
	maxUses := 12
	urlValues := make(goUrl.Values)
	urlValues = c.encodePrivateURL(urlValues, "", maxUses, nil, invite)

	var newChannel Channel
	loadedMaxUses, err := newChannel.decodePrivateURL(urlValues, "", invite)
	require.NoError(t, err)
...
func TestChannel_decodeInviteURL(t *testing.T) {
	...
	var newChannel Channel
	expectedErr := strings.Split(decodeEncryptedErr, "%")[0]
	_, err := newChannel.decodePrivateURL(urlValues, "", invite)
	require.ErrorContains(t, err, expectedErr)
}
...
func TestChannel_encodeSecretInviteURL_decodeSecretInviteURL(t *testing.T) {
	...
	maxUses := 2
	urlValues := make(goUrl.Values)
	urlValues = c.encodeSecretURL(urlValues, "", maxUses, nil, invite)

	var newChannel Channel
	loadedMaxUses, err := newChannel.decodeSecretURL(urlValues, "", invite)
	require.NoError(t, err)
