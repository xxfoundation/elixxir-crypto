...
// TestGenerateSharedKey tests the *normal* functioning of the function GenerateSharedKey.
func TestGenerateSharedKey(t *testing.T) {
	...
	//Trigger the jww warning
	if GenerateSharedKey(&g[0], baseKey[0], recursiveKeys[0], outSharedKey, outSharedKeyStorage) != outSharedKey {
...
// TestGenerateSharedKey2 tests if panic is thrown when recursiveKey size is too big
func TestGenerateSharedKey2(t *testing.T) {
	...
	g := cyclic.NewGroup(prime, cyclic.NewInt(55), cyclic.NewInt(33), cyclic.NewRandom(cyclic.NewInt(2), cyclic.NewInt(1000)))

	// 65536 bits for the long key
	outSharedKeyStorage := make([]byte, 0, 8192)

	//set recursiveKey to a size that triggers panic
	recursiveKeys := cyclic.NewIntFromString(
		"ef9ab83927cd2349f98b1237889909002b897231ae9c927d1792ea0879287ea3ffffffffff",
		16)
...
// TestGenerateSharedKey2 tests if panic is thrown when baseKey size is too big
func TestGenerateSharedKey3(t *testing.T) {
	...
	//set recursiveKey back to an acceptable size
	recursiveKeys := cyclic.NewIntFromString(
		"ef9ab83927cd2349f98b1237889909002b897231ae9c927d1792ea0879287ea3",
		16)
	...
	//set baseKey to a size that triggers panic
	baseKey := cyclic.NewIntFromString(
