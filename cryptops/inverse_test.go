...
// Tests that Inverse conforms to the cryptops interface
func TestInversePrototype_CryptopsInterface(t *testing.T) {
	var emptyInterface interface{}
	var cryptop Cryptop

	emptyInterface = Inverse
	cryptop, ok := emptyInterface.(Cryptop)
	_, ok2 := cryptop.(InversePrototype)

	if !(ok && ok2) {
		t.Errorf("InversePrototype() does not conform to the cryptops interface")
	}
}

// Tests that GetInputSize() returns the correct minimum input size.
func TestInversePrototype_GetInputSize(t *testing.T) {
	expect := uint32(1)
	actual := Inverse.GetInputSize()
...
func TestInversePrototype_GetName(t *testing.T) {
	expect := "Inverse"
	actual := Inverse.GetName()
...
// Tests the correctness of mathematical properties of inverse under the group.
func TestInversePrototype_MathProp(t *testing.T) {
	...
	// raise to power of totient(p) - 1; ie Inv(n)=n^(9) mod 11
	testVals := [][]*cyclic.Int{
		{grp.NewInt(4), grp.NewInt(1), grp.NewInt(3)}, // 4*3 = 1 mod 11
		{grp.NewInt(5), grp.NewInt(1), grp.NewInt(9)}, // 5*9 = 1 mod 11
		{grp.NewInt(9), grp.NewInt(1), grp.NewInt(5)}, // 9*5 = 1 mod 11
		{grp.NewInt(3), grp.NewInt(1), grp.NewInt(4)}, // 3*4 = 1 mod 11
		{grp.NewInt(1), grp.NewInt(1), grp.NewInt(1)}, // 1*1 = 1 mod 11
	}

	for _, val := range testVals {
		result := Inverse(grp, val[0], val[1])
